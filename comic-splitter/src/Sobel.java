/**
 *	Classe sobel com algoritmo de deteção de bordas em tons de cinza
 * 
...
class Sobel {
	...
	static void detectEdges(IImage image) {
			...
			for (int y = (templateSize - 1) / 2; y < height - (templateSize + 1) / 2; y++) {
				...
				for (int x1 = 0; x1 < templateSize; x1++) {
					for (int y1 = 0; y1 < templateSize; y1++) {
						...
						float value = (input[y2 * width + x2] & ImageUtils.IMAGE_OPERATOR) * (template[y1 * templateSize + x1]);
						sum += value;
					}
				}
				GY[y * width + x] = sum;
				for (int x1 = 0; x1 < templateSize; x1++) {
					for (int y1 = 0; y1 < templateSize; y1++) {
						int x2 = (x - (templateSize - 1) / 2 + x1);
						int y2 = (y - (templateSize - 1) / 2 + y1);
						float value = (input[y2 * width + x2] & ImageUtils.IMAGE_OPERATOR) * (template[x1 * templateSize + y1]);
						sum += value;
					}
				}
				GX[y * width + x] = sum;
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				output[y * width + x] = ImageUtils.BLACK | (sum << 16 | sum << 8 | sum);
			}
		}
